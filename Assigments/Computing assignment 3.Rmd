...
block_design_v2 = function(N, block_size, attenuation = 1, test){
  ...
  if (test == "Chi") { # Chi-squared
    tbl = table(Y$response, Y$arm)
    test = chisq.test(tbl)
    return(test$p.value)
  } else { # Mantel-Haenszel
    data <- table(Y$arm, Y$response, Y$block)
...
```{r}
set.seed(1)
trials.4a = sapply(1:sims, block_design_v2, N = 120, block_size = 30, test = "Chi")

rej = mean(trials.4a < 0.05)
```
...
```{r}
set.seed(1)
trials.4b = sapply(1:sims, block_design_v2, N = 120, block_size = 30, test = "MH")

rej = mean(trials.4b < 0.05)
```

By contrast, under a Mantel-Haenzel test, we reject the null `r round(rej*100, 2)`\% of the time. 