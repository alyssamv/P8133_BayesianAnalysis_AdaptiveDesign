...
decision.20 = s[which(dec == TRUE)[1]]; decision.20
...
decision.40 = s[which(dec == TRUE)[1]]; decision.40
...
decision.60 = s[which(dec == TRUE)[1]]; decision.60
...
decision = s[which(dec == TRUE)[1]]; decision
```
...
* At 20 subjects, stop the trial if <`r decision.20` subjects respond.
* At 40 subjects, stop the trial if <`r decision.40` subjects respond.
* At 60 subjects, stop the trial if <`r decision.60` subjects respond.
* At the end of the trial (71 subjects), stop the trial if <`r decision` subjects respond.
...
B = list(interim = c(),
         futility = c())
for (i in 1:N) {
  subj = rbinom(ntotal, 1, pe.true)
  if (sum(subj[1:20]) < decision.20) {
    B$interim[i] = 20
    B$futility[i] = TRUE
  } else if (sum(subj[1:40]) < decision.40) {
    B$interim[i] = 40
    B$futility[i] = TRUE
  } else if (sum(subj[1:60]) < decision.60) {
    B$interim[i] = 60
    B$futility[i] = TRUE
  } else if (sum(subj) < decision) {
    B$interim[i] = 71
    B$futility[i] = TRUE
  } else {
    B$interim[i] = NA
