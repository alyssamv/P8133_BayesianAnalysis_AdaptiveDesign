...
three.sims.alt = lapply(response.alt, function(i){
...
crm.sims.alt = lapply(response.alt, function(i){
  lapply(1:N, function(j) {
    pipeline(p.true = dlt.true,
             n.dose = n.dose,
             p.response = i,
...
start = Sys.time()
efftox.sims.alt = foreach(yyy = 1:length(response.alt)) %dopar% {
  pipeline(p.true = dlt.true,
           n.dose = n.dose,
           p.response = response.alt[[yyy]],
           p1.design = "efftox",
...
save.image(file = file.path(getwd(), "term project/sims_alt.RData"))
